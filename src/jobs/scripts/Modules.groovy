...
class Modules {
    ...
    def static runIntegrationTests(context)
    {
        ...
        // create context
        context.withEnv(["VC_PLATFORM=http://localhost:${platformPort}", "VC_STOREFRONT=http://localhost:${storefrontPort}", "VIRTO_CONN_STR_VirtoCommerce=Data Source=http://localhost:${sqlPort};Initial Catalog=VirtoCommerce2;Persist Security Info=True;User ID=sa;Password=v!rto_Labs!;MultipleActiveResultSets=True;Connect Timeout=30" ]) {
    ...
    def static prepareTestEnvironment(context)
    {
        ...
        def packagesDir = Utilities.getArtifactFolder(context)
        def allModulesDir = "c:\\CICD\\Artifacts\\"
